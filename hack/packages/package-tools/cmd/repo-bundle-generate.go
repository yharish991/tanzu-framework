...
func generatePackageBundlesSha256(projectRootDir, localRegistry string) error {
	...
	for i, pkg := range repository.Packages {
		...
		packageValues.Repositories[packageRepository].Packages[i].Version = pkgVersion
...
func generateRepoBundle(projectRootDir string) error {
	...
	for i := range repository.Packages {
		if err := generatePackage(projectRootDir, toolsBinDir, &repository.Packages[i]); err != nil {
...
func generatePackage(projectRootDir, toolsBinDir string, pkg *Package) error {
