...
VERSION=${MANAGEMENT_PACKAGE_REPO_VERSION}
...
function create_package_bundles() {
	...
	while IFS='|' read -r name version packageSubVersion path; do
		packageVersion="$version+$packageSubVersion"
		imagePackageVersion="v${version}_${packageSubVersion}"
...
function create_package_repo_bundles() {
	...
	while IFS='|' read -r name version packageSubVersion path packageCROverlay packageMetadataCROverlay; do
		...
		${packageYttCmd} > "${MANAGEMENT_PACKAGES_BUILD_ARTIFACTS_DIR}/package-repo-bundles/${PACKAGE_REPOSITORY}/packages/${name}.${domain}/${version}+${packageSubVersion}.yml"
	...
	tar -czvf "tkg-${PACKAGE_REPOSITORY}-repo-${VERSION}.tar.gz" -C "${PACKAGE_REPOSITORY}" .
	mv "tkg-${PACKAGE_REPOSITORY}-repo-${VERSION}.tar.gz" "${PACKAGE_REPOSITORY}"/
	popd
}
...
function push_package_repo_bundles() {
	mkdir -p "${MANAGEMENT_PACKAGES_BUILD_ARTIFACTS_DIR}/package-repo-bundles/${PACKAGE_REPOSITORY}/tkg-${PACKAGE_REPOSITORY}-repo-${VERSION}"
	tar -xvf "${MANAGEMENT_PACKAGES_BUILD_ARTIFACTS_DIR}/package-repo-bundles/${PACKAGE_REPOSITORY}/tkg-${PACKAGE_REPOSITORY}-repo-${VERSION}.tar.gz" -C "${MANAGEMENT_PACKAGES_BUILD_ARTIFACTS_DIR}/package-repo-bundles/${PACKAGE_REPOSITORY}/tkg-${PACKAGE_REPOSITORY}-repo-${VERSION}"
        "${TOOLS_BIN_DIR}"/imgpkg push -b "${REGISTRY}/${PACKAGE_REPOSITORY}:${VERSION}" --file "${MANAGEMENT_PACKAGES_BUILD_ARTIFACTS_DIR}/package-repo-bundles/${PACKAGE_REPOSITORY}/tkg-${PACKAGE_REPOSITORY}-repo-${VERSION}" --lock-output "${MANAGEMENT_PACKAGES_BUILD_ARTIFACTS_DIR}/package-repo-bundles/${PACKAGE_REPOSITORY}/tkg-${PACKAGE_REPOSITORY}-repo-${VERSION}/tkg-${PACKAGE_REPOSITORY}-repo-${VERSION}-lock-output.yaml"

	REPO_URL=$("${TOOLS_BIN_DIR}"/yq e ".bundle.image" "${MANAGEMENT_PACKAGES_BUILD_ARTIFACTS_DIR}/package-repo-bundles/${PACKAGE_REPOSITORY}/tkg-${PACKAGE_REPOSITORY}-repo-${VERSION}/tkg-${PACKAGE_REPOSITORY}-repo-${VERSION}-lock-output.yaml")
	REPO_VERSION_SHA=$(echo "${REPO_URL}" | cut -d ':' -f '2')
	"${TOOLS_BIN_DIR}"/ytt -f hack/packages/templates/repo-utils/packagerepo-tmpl.yaml -f hack/packages/templates/repo-utils/package-helpers.lib.yaml -f package-values-sha256.yaml -v packageRepository="${PACKAGE_REPOSITORY}" -v "${PACKAGE_REPOSITORY}PackageRepository.registry=${REGISTRY}" -v "${PACKAGE_REPOSITORY}PackageRepository.sha256=${REPO_VERSION_SHA}" > "${MANAGEMENT_PACKAGES_BUILD_ARTIFACTS_DIR}/package-repo-bundles/${PACKAGE_REPOSITORY}/tkg-${PACKAGE_REPOSITORY}-repo-${VERSION}.yaml"
	rm -rf "${MANAGEMENT_PACKAGES_BUILD_ARTIFACTS_DIR}/package-repo-bundles/${PACKAGE_REPOSITORY}/tkg-${PACKAGE_REPOSITORY}-repo-${VERSION}"
}
